...
public class TestClass {
    private static final String TEMPLATE = "C:\\Users\\gabil\\Desktop\\teste\\Scenarios\\Notepad\\variants-naousar\\variants-usar\\Open.config\\Notepad";//"C:\\Users\\gabil\\Desktop\\teste\\Scenarios\\Sudoku\\7\\Sudoku";//"C:\\Users\\gabil\\Desktop\\teste\\ArgoUML\\ArgoUML";//"C:\\Users\\gabil\\Desktop\\teste\\Notepad";

    private static final String VARIANTS = TEMPLATE;//"C:\\Users\\gabil\\Desktop\\teste\\Scenarios\\Sudoku\\7\\Sudoku";//"C:\\Users\\gabil\\Desktop\\teste\\Sudoku\\Sudoku";//"C:\\Users\\gabil\\Desktop\\teste\\Notepad";

    private static final String RESULTS = VARIANTS+File.separator+"results";//"C:\\Users\\gabil\\Desktop\\teste\\Scenarios\\Sudoku\\7\\Sudoku\\results";//"C:\\Users\\gabil\\Desktop\\teste\\Sudoku\\Sudoku\\results";//"C:\\Users\\gabil\\Desktop\\teste\\results";

    public static void main(String[] args) throws IOException {
        System.out.println(RESULTS);
        Project project = new Project("Notepad");//new Project("Sudoku");
        project.setSourceTypesToHandle(JavaSourceType.getInstance());

        File fullDir = new File(TEMPLATE);

        Set<Artifact> fullArtifacts = SourceParser.parseSources(new File(fullDir, "src\\Notepad"), project);//SourceParser.parseSources(new File(fullDir, "src\\p"), project);

        File results = new File(RESULTS);
        File variants = new File(VARIANTS);
        //for (File variantDir : variants.listFiles()) {
        if (variants.isDirectory()) {
            File resultDir = new File(results, variants.getName());
            resultDir.mkdirs();
            analyseCoverage(project, fullArtifacts, variants, resultDir);
        }
        //}
    }
    ...
    private static void analyseCoverage(Project project, Set<Artifact> fullArtifacts, File variantDir, File resultDir) throws IOException {
        Set<Artifact> varArtifacts = SourceParser.parseSources(new File(variantDir, "src\\Notepad"), project);//SourceParser.parseSources(new File(variantDir, "src\\p"), project);

        File jacocoReport = new File(variantDir, "jacoco.exec");
        File binDir = new File(variantDir, "bin\\Notepad");//new File(variantDir, "bin\\p");
        ...
        for (Map.Entry<String, ArrayList<Integer>> entry : linesMethodoCover.entrySet()) {
            File runtimeDir = new File(resultDir, "FilesRuntime");
            runtimeDir.mkdirs();
            File f = new File(runtimeDir, entry.getKey().substring(0, entry.getKey().lastIndexOf(".")) + ".runtime");
            if (!f.exists()) {
