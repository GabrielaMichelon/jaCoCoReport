...
public class ComputeStatementCoverage {
    ...
    private static final String TEMPLATE = "C:\\Stefan_Paper\\Paper\\2019-conf-Teclo-paper\\CaseStudies\\ArgoUML\\ArgoUML_Template";

    private static final String VARIANTS = "C:\\Users\\gabil\\Desktop\\BonusWork\\Results\\original_with_jacoco\\Tests_individual_features\\";

    private static final String RESULTS = "C:\\Users\\gabil\\Desktop\\BonusWork\\Results\\original_with_jacoco\\Tests_individual_features_COVERAGE_RESULTS";//(new File(System.getProperty("user.home")), "StackSPL_Coverage").getAbsolutePath();
    ...
    private static void parserArgoUMLVariant(File dir, Project project, Set<Artifact> artifacts){
        File appSrc = new File(dir, "src\\argouml-app\\src");
        if(appSrc.exists()){
            Set<Artifact> moduleArtifacts = SourceParser.parseSources(appSrc, project);
            artifacts.addAll(moduleArtifacts);
        }
        File sequenceSrc = new File(dir, "src\\argouml-core-diagrams-sequence2\\src");
        if(sequenceSrc.exists()){
            Set<Artifact> moduleArtifacts = SourceParser.parseSources(sequenceSrc, project);
            artifacts.addAll(moduleArtifacts);
        }
        File modelSrc = new File(dir, "src\\argouml-core-model\\src");
        if(modelSrc.exists()){
            Set<Artifact> moduleArtifacts = SourceParser.parseSources(modelSrc, project);
            artifacts.addAll(moduleArtifacts);
        }
        File eumlSrc = new File(dir, "src\\argouml-core-model-euml\\src");
        if(eumlSrc.exists()){
            Set<Artifact> moduleArtifacts = SourceParser.parseSources(eumlSrc, project);
            artifacts.addAll(moduleArtifacts);
        }
        File mdrSrc = new File(dir, "src\\argouml-core-model-mdr\\src");
        if(mdrSrc.exists()){
            Set<Artifact> moduleArtifacts = SourceParser.parseSources(mdrSrc, project);
            artifacts.addAll(moduleArtifacts);
        }
    }
    ...
    private static JacocoCoverageReport parserArgoUMLVariantCoverage(File dir) throws IOException {
        ...
        File app = new File(dir, "src\\argouml-app\\build\\");
        if(app.exists()){
            File jacocoReport = new File(app, "tests\\reports\\jacoco.exec");
            File binDir = new File(app, "classes");
        ...
        File sequence = new File(dir, "src\\argouml-app\\build\\");
        if(sequence.exists()){
            File jacocoReport = new File(sequence, "tests\\reports\\jacoco.exec");
            File binDir = new File(sequence, "classes");
            if(jacocoReport.exists() && binDir.exists()) {
                JacocoCoverageReport report = JacocoReportGenerator.parseExecFile(jacocoReport, binDir);
                packages.addAll(report.getPackages());
                sessions.addAll(report.getSessions());
        ...
        File euml = new File(dir, "src\\argouml-core-model-euml\\build\\");
        if(euml.exists()){
            File jacocoReport = new File(euml, "tests\\reports\\jacoco.exec");
            File binDir = new File(euml, "classes");
            if(jacocoReport.exists() && binDir.exists()) {
                JacocoCoverageReport report = JacocoReportGenerator.parseExecFile(jacocoReport, binDir);
                packages.addAll(report.getPackages());
                sessions.addAll(report.getSessions());
        ...
        File mdr = new File(dir, "src\\argouml-core-model-mdr\\build\\");
        if(mdr.exists()){
            File jacocoReport = new File(mdr, "tests\\reports\\jacoco.exec");
            File binDir = new File(mdr, "classes");
            if(jacocoReport.exists() && binDir.exists()) {
                JacocoCoverageReport report = JacocoReportGenerator.parseExecFile(jacocoReport, binDir);
                packages.addAll(report.getPackages());
                sessions.addAll(report.getSessions());
...
////                        }
        ...
        for (Map.Entry<String, ArrayList<Integer>> entry : linesMethodoCover.entrySet()) {
            File f = new File(resultDir, entry.getKey().substring(0,entry.getKey().lastIndexOf("."))+".runtime");
            if(!f.exists()){
